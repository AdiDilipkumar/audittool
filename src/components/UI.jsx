        ...
        {tiers.map((tier, i) => {
          ...
          return (
            <div key={tier.key} style={{
              ...
              {signed
                ? <span style={{ fontSize: 12, color: 'var(--status-green)', fontWeight: 500 }}>Signed {new Date(tier.signed_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</span>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 24px', borderBottom: '1px solid var(--border)' }}>
          ...
          <button onClick={onClose} style={{ cursor: 'pointer', color: 'var(--text-muted)', fontSize: 20, lineHeight: 1, background: 'none', border: 'none' }}>×</button>
        </div>
...
// ── New Audit Modal ────────────────────────────────────────────────────────
export function NewAuditModal({ onClose, onSubmit }) {
  const [fields, setFields] = useState({
    title: '', entity: '', audit_type: 'Assurance',
    lead_auditor_id: 'u-001', reviewer_id: 'u-002',
    planned_start: '', planned_end: '', period_under_review: '',
  });
  ...
  return (
    <Modal title="New Audit" onClose={onClose} width={540}>
...
// ── CommentButton ──────────────────────────────────────────────────────────
// Renders the 💬 icon + open count badge on section headers and row items.
// Props:
//   sectionRef  — string matching comment.sectionRef
//   rowRef      — string matching comment.rowRef, or null for section-level
//   comments    — full comments array for this audit
//   onClick     — opens the drawer
export function CommentButton({ sectionRef, rowRef = null, comments = [], onClick }) {
  const openCount = comments.filter(c =>
    c.sectionRef === sectionRef &&
    (rowRef ? c.rowRef === rowRef : true) &&
    c.status === 'Open'
  ).length;
  ...
  return (
    <button
      ...
      title={`${totalCount} comment${totalCount !== 1 ? 's' : ''} on this ${rowRef ? 'item' : 'section'}`}
      style={{
...
// ── CommentDrawer ──────────────────────────────────────────────────────────
// Fixed-position right-side drawer. Rendered once in App.jsx.
// Props:
//   isOpen, onClose, title, contextLabel
//   sectionRef, rowRef, auditId
//   comments, setComments, currentUser
export function CommentDrawer({ isOpen, onClose, title, contextLabel, sectionRef, rowRef, auditId, comments, setComments, currentUser }) {
  const [showForm, setShowForm]     = useState(false);
  const [newText, setNewText]       = useState('');
  const [assignTo, setAssignTo]     = useState('u-001');
  ...
  // Filter to this section/row
  const threads = comments.filter(c =>
    c.sectionRef === sectionRef &&
    (rowRef ? c.rowRef === rowRef : !c.rowRef || c.rowRef === null)
  );
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.45)', marginBottom: 3 }}>
              {contextLabel || 'Review comments'}
            </div>
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              padding: '5px 12px', background: 'var(--ni-teal)', color: '#fff',
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {showForm && (
            <div style={{
              ...
              <div style={{ padding: '10px 14px', fontSize: 12, fontWeight: 600, color: 'var(--ni-teal)', borderBottom: '1px solid var(--border)', background: 'var(--ni-teal-dim)' }}>
                New comment
              </div>
              <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>Comment</label>
                  <textarea
                    value={newText}
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>Assign to</label>
                  <select value={assignTo} onChange={e => setAssignTo(e.target.value)}
                    style={{ width: '100%', padding: '7px 10px', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', fontSize: 12, fontFamily: 'var(--font-sans)', background: 'var(--surface-0)', color: 'var(--text-primary)' }}>
                    {USERS.map(u => <option key={u.id} value={u.id}>{u.full_name} ({u.role})</option>)}
                  </select>
                </div>
              </div>
            ...
            <div key={thread.id} style={{
              ...
              <div style={{ padding: '10px 14px', display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                <div style={{
                  ...
                  background: thread.status === 'Closed' ? 'var(--text-muted)' : 'var(--ni-navy)',
              ...
              <div style={{ padding: '0 14px 10px 52px', fontSize: 12, color: 'var(--text-primary)', lineHeight: 1.6 }}>
                {thread.comment_text}
              </div>
                ...
                <div style={{ borderTop: '1px solid var(--border)', padding: '8px 14px', background: 'var(--surface-0)' }}>
                  ...
                  {thread.status === 'Open' && (
                    <div style={{ marginBottom: 8 }}>
                      <textarea
                        value={responseText[thread.id] || ''}
                        onChange={e => setResponseText(prev => ({ ...prev, [thread.id]: e.target.value }))}
                        rows={2} placeholder="Add response..."
                        style={{ width: '100%', padding: '6px 10px', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', fontSize: 12, fontFamily: 'var(--font-sans)', resize: 'none', background: 'var(--surface-0)', color: 'var(--text-primary)', marginBottom: 6 }}
                      />
                      <Button variant="secondary" size="sm" onClick={() => handleRespond(thread.id)}>Submit response</Button>
                    </div>
                  )}
                  <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                    <button onClick={() => handleClose(thread.id)} style={{
